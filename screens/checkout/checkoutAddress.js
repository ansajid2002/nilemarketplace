...
const Checkoutaddress = ({ navigation }) => {
  ...
  useEffect(() => {
    if (Object.keys(currentAddress).length > 0) {
      // console.log(currentAddress, customerAddressData);
          ...
          <View className="">
            {
              customerAddressData?.map((single) => {
                ...
                return (
                  <TouchableOpacity key={address_id} className="p-2 px-3  my-1 border bg-white border-gray-300 " onPress={debounce(() => handleAddressSelect(address_id), 500)}>
